...
PlayingCard::PlayingCard(Suit s, Rank r)
{
...
: CARD_SIZE(size), CARD_POSITION(position), SUIT_SCALE(suitScale), PADDING(padding), CHAR_SIZE(charSize)
{
    if (!metaLoaded) setupTexturesAndFonts();
    setupCard(s, r);
}
...
void PlayingCard::setupCard(Suit s, Rank r)
{
    ...
    suit.setTexture(*(suitTextures + s));
    suit.setScale(SUIT_SCALE);

    sf::Text text(*(ranks + r), PlayingCard::font);
    text.setStyle(sf::Text::Bold);
    text.setCharacterSize(CHAR_SIZE);
    if (s == Card::HEART || s == Card::DIAMOND) text.setFillColor(sf::Color::Red);
...
void PlayingCard::updatePosition()
{
    ...
    sf::FloatRect suitBounds = suit.getGlobalBounds();
    suit.setPosition({cardBounds.left + cardBounds.width/2 - suitBounds.width/2, cardBounds.top + cardBounds.height/2 - suitBounds.height/2});
